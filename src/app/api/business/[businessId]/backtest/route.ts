...
export async function GET(
  ...
  try {
    ...
    // Mock fallback — only when all real data sources fail
    if (dailyCandles.length < 5) {
      usedMockData = true;
    ...
    // ── Build dataQuality report ──
    const dataQuality = {
      dataSource: usedMockData ? 'mock' : type === 'crypto' ? 'coingecko' : 'yahoo',
      totalCandles: dailyCandles.length,
      dateRange: {
        start: dailyCandles[0]?.date || '',
        end: dailyCandles[dailyCandles.length - 1]?.date || '',
      },
      isMock: usedMockData,
      coingeckoFailed: type === 'crypto' ? coingeckoFailed : undefined,
      candlesUsedForStrategy: dailyCandles.length,
      candlesInDisplayRange: trimmedWeekly.reduce((sum, w) => sum + 7, 0),
    };
    ...
    if (usedMockData) {
      warnings.push(
        `Mock data was used because no real price data could be fetched for ${symbol}. ` +
        'Results are based on simulated prices and should not be used for investment decisions. ' +
        'Please try again later or use a different symbol.'
      );
    } else if (coingeckoFailed) {
      warnings.push(
        'CoinGecko API was unavailable or returned insufficient data. ' +
        'Data was fetched from an alternative source (Yahoo Finance). ' +
        'For best results, please try again later when CoinGecko rate limits reset.'
      );
    }
    if (trades.length === 0 && enoughData) {
      const stratLabel = activeStrategy === 'all' ? 'smartmoney' : activeStrategy;
      warnings.push(
        `No trades were generated by the ${stratLabel} strategy. ` +
        (dailyCandles.length < 50
          ? `Only ${dailyCandles.length} candles were available — this strategy requires at least 50 candles for proper indicator warmup (SMA50, ADX, etc.). ` +
            'Try extending the date range or selecting a different asset with more history.'
          : `${dailyCandles.length} candles were analyzed but no entry signals met the strategy thresholds. ` +
            'This could mean the market conditions didn’t align with the strategy requirements during this period. ' +
            'Try a different date range, a different asset, or a different strategy (e.g., Breakout for volatile markets or RSI for range-bound markets).'
        )
      );
    }
