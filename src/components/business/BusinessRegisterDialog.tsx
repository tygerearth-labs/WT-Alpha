...
export default function BusinessRegisterDialog({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMode(category === 'bisnis' ? 'bisnis' : 'investasi');
      ...
      <DialogContent className="bg-[#1A1A2E] border-white/[0.06] text-white sm:max-w-[480px]">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <Label className="text-white/80">{t('biz.categoryName')} *</Label>
            <Select value={category} onValueChange={(val) => setCategory(val as 'bisnis' | 'investasi')}>
              <SelectTrigger className="bg-white/[0.05] border-white/[0.1] text-white">
                <SelectValue placeholder={t('biz.selectCategory')} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="bisnis">{t('biz.bisnis')}</SelectItem>
                <SelectItem value="investasi">{t('biz.investasi')}</SelectItem>
              </SelectContent>
            </Select>
          </div>
